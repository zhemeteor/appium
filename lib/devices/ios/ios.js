...
class IOS extends Device {
  ...
  async _installToRealDevice () {
    ...
    if (this.args.autoLaunch === false) {
      ...
      if (this.args.udid) {
        let installed;
        try {
          installed = await this._isAppInstalled(this.args.bundleId);
        } catch (err) {
          installed = false;
        }
        if (!installed) {
          ...
          if (this.args.ipa && this.args.bundleId) {
            ...
            await this._installApp(this.args.app);
          } else {
  ...
  async cleanupSimState () {
    if (this.realDevice && this.args.bundleId && this.args.fullReset) {
      ...
      try {
        ...
        try {
          await this._removeApp(bundleId);
        } catch(err) {
  ...
  async _isAppInstalled (bundleId) {
    if (this.args.udid) {
      if (!this.realDevice) {
        this.realDevice = this.getIDeviceObj();
      }
      await B.fromNode(this.realDevice.isInstalled.bind(this.realDevice, bundleId));
    } else {
      throw new Error("You can not call isInstalled for the iOS simulator!");
    }
  }

  isAppInstalled (bundleId, cb) {
    B.resolve(this._isAppInstalled(bundleId)).nodeify(cb);
  }

  async _removeApp (bundleId) {
    if (this.args.udid) {
      if (!this.realDevice) {
        this.realDevice = this.getIDeviceObj();
      }
      ...
      await B.fromNode(this.sim.remove.bind(this.sim, bundleId));
    }
  }

  removeApp (bundleId, cb) {
    B.resolve(this._removeApp(bundleId)).nodeify(cb);
  }

  async _installApp (unzippedAppPath) {
    if (this.args.udid) {
      if (!this.realDevice) {
        this.realDevice = this.getIDeviceObj();
      }
      B.fromNode(this.realDevice.install.bind(this.realDevice, unzippedAppPath));
    } else {
      B.fromNode(this.sim.install.bind(this.sim, unzippedAppPath));
    }
  }

  installApp (unzippedAppPath, cb) {
    B.resolve(this._installApp(unzippedAppPath)).nodeify(cb);
  }

  async _startApp (args) {
    if (this.args.udid) {
      throw new Error("You can not call startApp for a real device!");
    } else {
      B.fromNode(this.sim.launch.bind(this.sim, args.appPackage));
    }
  }

  startApp (args, cb) {
    B.resolve(this._startApp(args)).nodeify(cb);
  }

  async _unpackApp (req) {
    B.fromNode(deviceCommon.unpackApp.bind(deviceCommon, req, '.app'));
  }

  unpackApp (req, cb) {
    B.resolve(this._unpackApp(req)).nodeify(cb);
  }
  ...
  async _initAutoWebview () {
    if (this.args.autoWebview) {
      logger.debug('Setting auto webview');
      await B.fromNode(this.navToInitialWebview.bind(this));
    }
  }

  initAutoWebview (cb) {
    B.resolve(this._initAutoWebview()).nodeify(cb);
  }

  async _getContextsAndViews () {
    let webviews = await B.fromNode(this.listWebFrames.bind(this));
  ...
  getContextsAndViews (cb) {
    B.resolve(this._getContextsAndViews()).nodeify(cb);
  }

  async getLatestWebviewContextForTitle (titleRegex) {
    let contexts = await this._getContextsAndViews();
    let matchingCtx;
    _(contexts).each((ctx) => {
  ...
  async navToViewThroughFavorites () {
    ...
    try {
      let res = await B.fromNode(this.findElement.bind(this, 'xpath', '//UIAScrollView[1]/UIAButton[1]'));
      if(res.status !== status.codes.Success.code) {
