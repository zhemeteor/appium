...
class IOS extends Device {
  ...
  async startRealDevice () {
    ...
    await B.fromNode(this.detectUdid.bind(this));
    await B.fromNode(this.parseLocalizableStrings.bind(this));
    await B.fromNode(this.setBundleIdFromApp.bind(this));
    await this.createInstruments();
    await B.fromNode(this.startLogCapture.bind(this));
    await B.fromNode(this.installToRealDevice.bind(this));
    await B.fromNode(this.startInstruments.bind(this));
    await this.onInstrumentsLaunch();
    await B.fromNode(this.configureBootstrap.bind(this));
    await this._setBundleId();
    await B.fromNode(this.setInitialOrientation.bind(this));
    await B.fromNode(this.initAutoWebview.bind(this));
    await B.fromNode(this.waitForAppLaunched.bind(this));
  }

  async startSimulator () {
    await removeInstrumentsSocket(this.sock);
    await B.fromNode(this.setXcodeVersion.bind(this));
    await B.fromNode(this.setiOSSDKVersion.bind(this));
    await B.fromNode(this.checkSimAvailable.bind(this));
    await B.fromNode(this.createSimulator.bind(this));
    await B.fromNode(this.moveBuiltInApp.bind(this));
    await B.fromNode(this.detectUdid.bind(this));
    await B.fromNode(this.parseLocalizableStrings.bind(this));
    await B.fromNode(this.setBundleIdFromApp.bind(this));
    await this.createInstruments();
    await B.fromNode(this.setDeviceInfo.bind(this));
    await B.fromNode(this.checkPreferences.bind(this));
    await B.fromNode(this.runSimReset.bind(this));
    await B.fromNode(this.isolateSimDevice.bind(this));
    await B.fromNode(this.setLocale.bind(this));
    await B.fromNode(this.setPreferences.bind(this));
    await B.fromNode(this.startLogCapture.bind(this));
    await B.fromNode(this.prelaunchSimulator.bind(this));
    await B.fromNode(this.startInstruments.bind(this));
    await this.onInstrumentsLaunch();
    await B.fromNode(this.configureBootstrap.bind(this));
    await this._setBundleId();
  ...
  instantLaunchAndQuit (needSafariDirs, cb) {
    ...
    this.prelaunchSimulator(function (err) {
      if (err) return cb(err);
      this.makeInstruments().nodeify(function (err, instruments) {
  ...
  setDeviceTypeInInfoPlist (cb) {
    ...
    B.resolve(updatePlistFile(plist, {UIDeviceFamily: [deviceTypeCode]})).nodeify(cb);
  }

  getBundleIdFromApp (cb) {
    logger.debug("Getting bundle ID from app");
    let plist = path.resolve(this.args.app, "Info.plist");
    B.resolve(parsePlistFile(plist)).nodeify(function (err, obj) {
      if (err) {
  ...
  prelaunchSimulator (cb) {
    ...
    xcode.getPath(function (err, xcodePath) {
      ...
      this.endSimulator(function (err) {
        ...
        let waitForSimulatorLogs = function (countdown) {
          if (countdown <= 0 ||
  ...
  parseLocalizableStrings (/* language, stringFile, cb */) {
    ...
    B.resolve(parsePlistFile(strings)).nodeify(function (err, obj) {
      if (err) {
